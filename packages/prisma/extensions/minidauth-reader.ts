...
/**
 * Mint a short-lived assertion that this already-authenticated user is the reader. Cal.com has already
 * verified the user (the id comes from the server session, never from the client); this token just
 ...
 */
export function mintCalReaderToken(uid: string): string {
...
export function withMinidauthReader<T>(uid: string | null | undefined, fn: () => T): T {
  if (!uid) return fn();
  return readerStore.run({ userToken: mintCalReaderToken(uid) }, fn);
}
